...
import '../other_widgets/generate_code_alert.dart';

class DraftPostAlert extends StatefulWidget {
  final PostModel draftPost;
  final int sectionLenght;
  const DraftPostAlert({super.key, required this.draftPost, required this.sectionLenght});

  @override
  State<DraftPostAlert> createState() => _DraftPostAlertState();
}

class _DraftPostAlertState extends State<DraftPostAlert> {
  ...
  void initState() {
    ...
    //initDraftInfo
    titleController.text = widget.draftPost.mainTitle;
    tags = widget.draftPost.tags ?? [];
    openedSectionIndex = widget.sectionLenght-1;
    if(widget.draftPost.mainImage != null && widget.draftPost.mainImage!.isNotEmpty){
      imageUploaded = widget.draftPost.mainImage;
    }
                                           ...
                                           MouseRegion(
                                                  ...
                                                  child: GestureDetector(
                                                    onTap: () async {
                                                      ...
                                                      if (file != null && mounted) {
                                                        var tempRef = await context.read<NewPostProvider>().uploadPostMainImage(widget.draftPost.pid, file, "mainImage");
                                                        setState(() {
      ...
      builder: (context) => AlertDialog(
        ...
        if (value == "private")  {
          ...
          if(result != null && mounted) {
            await context.read<NewPostProvider>().publishPost(isPrivate: true, destinationUserId: result, mainImageRef: imageUploaded);
            if(mounted){
          ...
          await context.read<NewPostProvider>().publishPost(isPrivate: false, mainImageRef: imageUploaded );
          if(mounted){
              context.read<AuthProvider>().fetchUser();
              AutoRouter.of(context).pop();
            }
          ...
          if (code != null && mounted) {
            await context.read<NewPostProvider>().publishPostWithCode(mainImageRef: imageUploaded, reedemableCode: code);
             if(mounted){
              context.read<AuthProvider>().fetchUser();
              AutoRouter.of(context).pop();
            }
